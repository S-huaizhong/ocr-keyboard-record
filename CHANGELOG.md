...
### Fixed
- 三个核心全局快捷键从 `keyboard` 低级钩子迁移到托盘窗口持有的 Win32 `RegisterHotKey` / `WM_HOTKEY` 消息，避免长时间运行、会话切换或钩子线程异常后快捷键失效；使用 `MOD_NOREPEAT` 防止按住时重复触发。
- 新建文本窗口恢复完整的 Windows 标准标题栏、边框、阴影、拖动、缩放、最小化和关闭行为；删除编辑器专用 WndProc 子类化、DWM 框架扩展、非客户区命中测试和 Tk 自绘拖动路径，消除拖动黑框、无法移动及原生消息重入崩溃。文件菜单、字数、编辑、滚动、保存/另存为与关闭确认功能保持不变。
