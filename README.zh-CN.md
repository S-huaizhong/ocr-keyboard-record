...
| 输入记录面板 | 新建文本窗口 | 托盘图标与右键菜单 |
|---|---|---|
| ![输入记录面板](./docs/screenshots/04-input-panel-zh.png) | ![新建文本窗口](./docs/screenshots/06-new-text-editor-en.png) | ![托盘右键菜单](./docs/screenshots/05-tray-menu-en.png) |
...
- 1.1.4 起从文件尾部按需分页，大日志不会在打开面板时全量载入内存。

### 5. 托盘图标与语言切换

![托盘右键菜单](./docs/screenshots/05-tray-menu-en.png)

- 托盘图标左键 = 触发屏幕 OCR；右键 = 完整菜单（识别屏幕 / 输入记录 / 新建文本文档 / 选择语言 / 退出）。
- “选择语言”子菜单即时切换整个界面 + 快捷键提示 + 托盘菜单标签的中英文，无需重启。

### 6. 内置文本编辑器

![新建文本窗口](./docs/screenshots/06-new-text-editor-en.png)

- 可通过 `Ctrl+Alt+N` 或托盘菜单的“新建文本文档”打开。
- 使用完整的 Windows 标准标题栏、边框、原生拖动、缩放、最小化和关闭行为。
- 保留实时字数、自动换行、14 号字、保存、另存为及未保存关闭确认。
...
- 托盘图标：使用无文字、只有横线的空白练习纸设计，16～64 像素分别按原生尺寸绘制；左键触发屏幕 OCR，右键菜单可识别屏幕、打开输入记录或新建文本文档
- 全局热键：`Ctrl+Alt+F` 打开 OCR 覆盖层，`Ctrl+Alt+X` 打开输入记录窗口，`Ctrl+Alt+N` 打开内置文本编辑器并自动切到最上层
- 内置文本编辑器：使用完整的 Windows 标准标题栏、边框、原生阴影、拖动、缩放、最小化和关闭行为；以 2K 下右上角锚点 `(2500, 50)` / 客户区 `600×600` 为基准（左上角为 `(1900, 50)`），随 1080p、4K 等分辨率等比缩放；字数位于文件工具栏，文本区内置按需显示的 70% 视觉透明滚动滑块；支持自动换行、14 号字、保存、另存为和关闭确认
...
自 1.1.4 起，关闭对应开关会阻止读取剪贴板或焦点控件内容，而不仅是阻止写入日志；程序同时使用单实例保护，避免重复钩子和重复记录。
...
以下问题受限于当前单文件 Python 架构、第三方 OCR 模型及 Windows 输入法内部实现，部分可能在未来的大版本重写中得到改善，另一部分本质属于架构外的局限。
...
### 2. 输入记录：中文输入法上屏文本通过 UI Automation 尽力而为地捕获

**当前状态（1.1.1 起）**：项目已内置基于 UI Automation 监听 `EditControl.TextChanged` 事件的中文上屏捕获，需在输入记录面板中手动勾选 **中文 IME 捕获** 开关。当它能正常工作时，日志行会直接记录为 `{"kind":"ime","text":"你好"}` —— 真实的上屏文本而非拼音。下面列出的局限仍适用于 UIA 无法触及的目标。

**现象（开关关闭或 UIA 不适用时）**：使用微软拼音、搜狗、QQ、Rime 等中文输入法输入"你好"时，输入记录面板可能显示为 `你hao1好`、`nihao1你好` 或 `n h a o 1 你好` 等混合形式。英文输入不受影响；其他语种输入法尚未测试。
...
- 因此 hook 单独只能观测到拼音字母（作为普通英文按键）与候选数字（作为选字快捷键），最终的中文需要另行重建—— 本项目 1.1.1 后通过 UI Automation 订阅焦点控件的 `TextChanged` 事件来完成。
- 另一条方案—— 注入目标进程并 hook `ImmGetCompositionString` 或 TSF 接口—— 涉及 DLL 注入与 UWP 沙盒兼容性问题，超出当前单文件 Python 脚本的架构承载范围。

**当前 UIA 实现无法覆盖的场景**：

- UWP 沙盒应用（商店应用、小组件）—— UIA 事件被沙盒拦截
- 未暴露标准 `Edit` pattern 的自绘输入控件（如 VS Code Webview、部分浏览器扩展、IDE 补全弹窗等）
- 游戏与全屏独占应用
- 未启用无障碍树的 Electron / Chromium WebView 内容
- 用户未开启该开关的默认情形
...
**技术原因**：DBNet 训练样本的宽高比分布相对集中，极端比例超出其适用区间。未来若迁移至更通用的检测模型，该问题将得到缓解。
