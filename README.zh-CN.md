...
- 内置文本编辑器：视觉上保持无框，但底层保留标准 Win32 顶层窗口样式并由 Windows DWM 绘制真实系统阴影（不创建额外阴影窗口），以 2K 下右上角锚点 `(2500, 50)` / `600×600` 为基准（左上角为 `(1900, 50)`），随 1080p、4K 等分辨率等比缩放；字数位于顶部文件栏，文本区内置按需显示的 70% 视觉透明滚动滑块；支持自动换行、14 号字、保存、另存为和关闭
